...
//  }
  ...
  pinMode(MOTOR_1, OUTPUT); // PWM
  pinMode(MOTOR_2, OUTPUT); // PWM
  pinMode(POT_IN, INPUT); // analog
  pinMode(BATT_IN, INPUT); // analog
...
void loop()
{
  ...
  if (runPID) {
    ...
    if (motor_speed > 0) {
      ...
      digitalWrite(MOTOR_2, LOW);
      analogWrite(MOTOR_1, -1 * motor_speed);
    }
  ...
  //------------------  BATTERY MONITOR  ------------------
  if (runBattery) {
    int batSense = analogRead(BATT_IN);
    float batVolt = 2.0 * batSense * (5.0 / 1023.0);
    if (batVolt < 7.49) {
      //Serial.println("LOW BATTERY WARNING (under 25%)");
